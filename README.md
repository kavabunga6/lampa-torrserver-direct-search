...
По умолчанию он отключает `CUB` и `AI-ассистент`, но оставляет `TMDB`. В `Настройки -> Источники поиска` каждый источник можно включить обратно или полностью выключить фильтр.
